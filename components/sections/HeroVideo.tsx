    ...
    >
      ...
      */}
      ...
      {/*
        ...
        色は必ず --color-void (#050506) に揃える。以前は rgba(9,9,9,…)
        という別値を直書きしていたため、lg未満(映像の枠がHero全体では
        なく自分のアスペクト比ボックスだけに収まる構図)で、映像枠の
        下端(このグラデーションが効く範囲)と、その下にある地の色
        (body の --color-void)との間にわずかな色差の継ぎ目ができていた
        (実測: 「レスポンシブだと背景の色がPC版と違う」という指摘の
        一因)。lg以上は枠がHero全体に広がるため継ぎ目自体が画面外に
        逃げ、気づかれていなかった。
      */}
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(5,5,6,0.8) 0%, rgba(5,5,6,0) 20%, rgba(5,5,6,0) 70%, rgba(5,5,6,0.92) 100%)",
        }}
      />
    </div>
  );
