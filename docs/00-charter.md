...
### 実測で判明した「Visionが壊れる経路」

Phase 4 で Baseline が実際に退行を2件検出した。想定していなかった経路が含まれる。

| 経路 | 内容 |
|---|---|
| **3Dの寸法変更** | 回廊の板を 9.5→22 に拡大したところ、同じ距離設定のまま Vision に薄い矩形が写り込んだ |
| **他セクションのCSS変更** | Narrative のキャプションの padding / max-width を変えただけで、**Vision の差分が最大47.8%になった**。ページ全体の高さが変わり、pin区間内のスクロール位置がずれたため |

**後者が重要**: Vision のコードを一切触らず、**別セクションの余白を数px変えただけ**で Vision の見え方が変わる。
`pin` + `scrub` の構成では、**ページ高の変化がそのまま pin区間内の進行度のズレになる**。

したがって **Baseline --check は「3Dを触ったとき」だけでなく、レイアウトに影響する全ての変更で実行する。**
`padding` や `max-width` の調整も対象に含める。
