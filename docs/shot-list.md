...
## Motion Regression Rule（Phase 4 以降・必須）

**1つのShotを修正したら、前後1Shotを含めた3Shotを必ず通しで確認する。**
...
| `H-03` | `H-02` / `H-03` / `H-04` |
| `W-06` | `W-05` / `W-06` / `W-07` |
| **継ぎ目を跨ぐ変更**(`L-08`・`H-06`・`W-09`・`F-01`) | **跨いだ両側のセクションを丸ごと** |

理由と検出手順は [00-charter.md](./00-charter.md#motion-regression-rulephase-4-以降) を参照。
**変更したShot自体は正常に見えるのに後続が崩れる**のが典型的な壊れ方なので、単体確認では検出できない。
