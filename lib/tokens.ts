...
export const vehicle = {
  ...
  hero: {
    /** 登場位置。画面右外。奥行きも少し付けて「走ってくる」感を出す */
    fromX: 9,
    fromZ: -1.5,
     ...
     * カメラは +Z 側にあるので、rotationY=0 だとリアがカメラを向く。
     * π 回してフロントを正対させ、そこから 0.62rad 振って
     * 斜め前から見た構図(3/4ビュー)にする。
     */
    toRotationY: Math.PI + 0.62,
    /** 登場時はもっと横を向いている(走ってきて向きを変えながら停まる) */
    fromRotationY: Math.PI + 1.35,
  ...
  philosophy: {
    ...
    toRotationY: Math.PI + 0.62 + Math.PI,
  ...
  sell: {
    ...
    gateCloseEnd: 0.25,
    /** 側面が見える角度(真横) */
    toRotationY: Math.PI + 0.62 + Math.PI * 0.5,
  ...
  /* ── Buy: カメラが車両を周回する ── */
  buy: {
    /** 周回の振れ幅(ラジアン)。1周させず、見せ場だけを通る */
    orbitSweep: Math.PI * 1.15,
  ...
  camera: {
    hero: { x: 0.4, y: 1.55, z: 8.6, lookY: 0.95 },
    philosophy: { x: -0.6, y: 1.75, z: 8.2, lookY: 1.05 },
    sell: { x: 0, y: 1.35, z: 7.8, lookY: 0.85 },
    buy: { x: 0, y: 1.8, z: 7.4, lookY: 0.9 },
    find: { x: 0, y: 1.45, z: 9.2, lookY: 0.95 },
    contact: { x: 0, y: 1.5, z: 9.6, lookY: 0.9 },
  },
