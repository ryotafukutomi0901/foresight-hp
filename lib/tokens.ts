 ...
 * STAGE — 車両を包む環境光
 *
 * 床は置かない。平面を敷くと地平線が画面を横切り、
 * 「暗闇に浮かぶ」というアートディレクションが壊れる（実測）。
 * 参照資料 vehicle180.png にも床は無い。
 * ──────────────────────────────────────────────────────────────── */

export const stage = {
  /** 環境マップの解像度。反射に使うだけなので低くてよい */
  envResolution: 256,
  /** 真上の面光源。ルーフ・ボンネットを起こす */
  envTop: 1.6,
  /** 左右の縦帯。ボディサイドを流れるハイライトを作る */
  envSide: 2.2,
  /** 前方の弱い面。ガラスとグリルに映り込む */
  envFront: 0.9,
} as const;
...
/* ────────────────────────────────────────────────────────────────
 * EDGES — 写実GLBに線画のトーンを被せるアウトライン
 * 出典: docs/vehicle-glb-handoff.md #3
 *
 * ブランドの参照資料は黒地に白1pxの線画。納品モデルは写実。
 * この2つを繋ぐのがアウトラインで、車体の「設計線」だけを
 * 白く出して線画のトーンに寄せる。
 * ──────────────────────────────────────────────────────────────── */
...
export const edges = {
   ...
   * 法線差の閾値。
   *
   * 写実テクスチャを使っていた頃は 0.8 まで上げないと
   * 微小な凹凸を拾って面が斑点だらけになった。
   * ボディをフラットな黒に差し替えた後は、その原因が消えたので
   * 0.42 まで下げて線の情報量を増やしている。
   * 参照PNG(vehicle180.png)はドアの分割線やホイールのスポークまで
   * 描かれた製図なので、線は多いほうが資料に近い。
   */
  normalThreshold: 0.42,
  /** 深度差の閾値。シルエットの出方を決める */
  depthThreshold: 0.0006,
  /**
   * 線の明るさ。Bloomの luminanceThreshold(0.62) を
   * 超える値にしないと線が発光しない。
   */
  strength: 1.15,
} as const;
...
export const scroll = {
  ...
  pinUnseenMobile: "+=280%",
  /*
   * 車両区間のpin長。セクションを画面に固定したまま、この距離だけ
   * スクロールさせて車両を動かす。
   *
   * viewport相対(%)にしているのは、コンテンツ量が変わっても
   * 進行度がズレないようにするため。過去にpaddingの変更だけで
   * Visionの進行度が47.8%ズレた事故があり、同じ罠を踏まない。
   *
   * Contactは含めない。フォームを固定すると、送信するまで
   * ページが進まなくなり、問い合わせの妨げになる。
   */
  vehiclePin: {
    /** 半周回転 + リアハッチ開放 + コピー出現。最も長い */
    philosophy: "+=240%",
    /** ハッチを閉じ、側面へ回り込み、スキャンが通る */
    sell: "+=180%",
    /** カメラの周回 */
    buy: "+=160%",
    /** 走行 + 光のライン分岐 */
...
export const vehicle = {
  ...
  camera: {
    ...
    hero: { x: 1.15, y: 1.5, z: 8.2, lookY: 0.95 },
    philosophy: { x: -0.65, y: 1.75, z: 7.9, lookY: 1.05 },
    /*
     * Sellの終端は、次のBuy区間の周回の始点(角度0 = 真正面)と
     * 一致させる。ここがズレると区間の継ぎ目でカメラが飛ぶ。
     * z は orbitRadius(7.4)、x は 0、y は orbitHeightFrom(0.95) に合わせる。
     */
    sell: { x: 0, y: 0.95, z: 7.4, lookY: 0.85 },
    buy: { x: 0, y: 1.7, z: 7.4, lookY: 0.95 },
    find: { x: 0, y: 1.55, z: 8.6, lookY: 0.95 },
    /*
     * Contactは引いて低く見せる。lookY を上げると被写体が
     * 画面の下側に落ち、右半分の問い合わせフォームに被らない。
     * ここは体験の余韻であって主張する場面ではないので、
     * 車は静かな背景として下に置く。
     */
    contact: { x: 0, y: 1.95, z: 11.2, lookY: 1.55 },
  },

  /*
   * 車体の姿勢。
   *
   * どれも小さい値であることが重要。実車の挙動をそのまま入れると
   * 「揺れる玩具」に見える。ブランドが求めているのは静けさなので、
   * 「動いたと気づかないが、無いと嘘っぽい」量に留める。
   */
  posture: {
    /** 旋回時の前輪の最大切れ角(rad)。約17度 */
    steerMax: 0.3,
    /** Find区間の蛇行の振れ幅(rad)。約4度。進路を探る気配だけ */
    steerWeave: 0.07,
    /**
     * ステア1radあたりの車体ロール量。
     * 0.12なら最大切れ角0.3radで約2度傾く。
     * これ以上だと船のように揺れて安っぽくなる。
     */
    rollPerSteer: 0.12,
    /** 制動時のノーズダイブ(rad)。約1.7度 */
    brakePitch: 0.03,
  },
